...
**Quick reference for resolving common development issues in ONE platform.**

---

## When to Use This Guide

**ALL agents should reference this guide when encountering:**
...
**How to use:** Search for symptoms, apply solution, verify fix, then continue with your task.
...
**Before creating mutations/queries:**
- Verify schema exists in `backend/convex/schema.ts`
- Check if indexes are defined for query patterns
- Validate groupId scoping for multi-tenancy

**If types are missing:**
- Run `cd backend && npx convex dev`
- Wait for `_generated/` directory to update
- Verify imports from `@/convex/_generated/api`

### agent-frontend

**Before creating components:**
- Check if shadcn/ui component exists: `bunx shadcn@latest add <component>`
- Verify Tailwind theme variables in `src/styles/global.css`
- Test SSR compatibility for browser APIs

**If hydration fails:**
- Add `client:load` or `client:only="react"` directives
- Move browser API usage inside `useEffect`
- Test with both SSR and CSR modes

### agent-quality

**Before defining tests:**
- Verify test structure matches existing patterns in `web/test/`
- Check if test fixtures exist for auth/session
- Validate test database isolation

**If tests fail:**
- Check environment variables in test config
- Verify Convex backend is running during integration tests
- Inspect test logs: `bun test --verbose`

### agent-ops

**Before deploying:**
- Verify all environment variables are set in Cloudflare dashboard
- Check build passes locally: `bun run build`
- Validate Convex deployment is up: `cd backend && npx convex deploy`

**If deployment fails:**
- Check Cloudflare Pages build logs
- Verify `CLOUDFLARE_GLOBAL_API_KEY` is set
- Test edge runtime compatibility

### agent-integrator

**Before connecting external systems:**
- Verify API credentials are in environment variables
- Check if integration already exists in `one/connections/`
- Validate protocol compliance (A2A, ACP, AP2, X402, AG-UI)

**If integration fails:**
- Check external API status/rate limits
...
1. **Check source documentation:**
   - `/one/knowledge/troubleshooting.md` (this file's source)
   - `/one/knowledge/guidelines.md` (development best practices)
   - `/one/knowledge/architecture.md` (platform structure)

2. **Search existing patterns:**
   - `grep -r "similar pattern" web/src/`
   - Check `one/connections/patterns.md`

3. **Ask user for clarification:**
   - Use `AskUserQuestion` tool
   - Provide specific options based on troubleshooting
   - Include context from error messages

4. **Document new solutions:**
   - If you discover a new fix, update `one/knowledge/troubleshooting.md`
   - Add to relevant agent documentation
   - Log in `one/events/` for future reference

---

**Golden Rule: When in doubt, regenerate types, clear cache, and rebuild.**

```bash
# The nuclear option (fixes 90% of mysterious issues)
cd backend && npx convex dev &
cd web && rm -rf .astro/ && bunx astro sync && bun run build
```
