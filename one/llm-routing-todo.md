...
- [x] **C2-R1** Recon: D1 schema, migration pattern `cache, schema` [low]
- [x] **C2-R2** Recon: intent.ts full API surface `cache, intent` [low]
- [x] **C2-D1** Decide: cache key strategy `cache, design` [medium]
- [x] **C2-D2** Decide: TTL + invalidation strategy `cache, design` [medium]
- [x] **C2-E1** Edit: D1 migration — response_cache + call_log `cache, schema` [low]
- [x] **C2-E2** Edit: cache check pre-LLM, cache store post-LLM `cache, wire` [medium]
- [x] **C2-E3** Edit: call_log per LLM call `cost, wire` [low]
- [x] **C2-V1** Verify: cache hit returns stored response `cache, test` [medium]
- [x] **C2-V2** Verify: call_log aggregates per tag/group `cost, test` [low]

### CYCLE 3: GROW — Intent Cache + Content Recall

- [x] **C3-R1** Recon: intent.ts API + D1 tables `intent, api` [low]
- [x] **C3-R2** Recon: recallHypotheses content search `recall, substrate` [low]
- [x] **C3-R3** Recon: inferDocsFromTags for nanoclaw `context, docs` [low]
- [x] **C3-D1** Decide: intent seeding strategy `intent, design` [medium]
- [x] **C3-D2** Decide: content recall scope `recall, design` [medium]
- [x] **C3-E1** Edit: D1 migration — intents + intent_queries `intent, schema` [low]
- [x] **C3-E2** Edit: resolveIntent before LLM call `intent, wire` [medium]
- [x] **C3-E3** Edit: contentRecall in substrate.ts `recall, wire` [medium]
- [x] **C3-E4** Edit: extend prompt.ts with recall + doc context `context, wire` [medium]
- [x] **C3-V1** Verify: intent resolution → cached response `intent, test` [medium]
- [x] **C3-V2** Verify: content recall enriches prompt `recall, test` [medium]

### CYCLE 4: SUSTAIN — Pattern Registry + Self-Measurement

- [x] **C4-R1** Recon: classify-fallback.ts migration path `pattern, classify` [low]
- [x] **C4-R2** Recon: outcome.ts pattern quality scoring `pattern, valence` [low]
- [x] **C4-D1** Decide: pattern registry design `pattern, design` [medium]
- [x] **C4-E1** Edit: D1 migration — claw_patterns `pattern, schema` [low]
- [x] **C4-E2** Edit: pattern match in router.ts `pattern, wire` [medium]
- [x] **C4-E3** Edit: pattern quality scoring in outcome.ts `pattern, measure` [medium]
- [x] **C4-V1** Verify: pattern match → reply/block/tag/route `pattern, test` [medium]
- [x] **C4-V2** Verify: self-measurement via valence `pattern, measure` [medium]
