...
export function RoleCatalog({ selected, onChange, exclude = ['ceo'] }: Props) {
  ...
  if (loading) return <div className="p-4 text-xs text-muted-foreground">Loading roles…</div>
  if (error) return <div className="p-4 text-xs text-destructive">Roles error: {error}</div>
  if (roles.length === 0) return <div className="p-4 text-xs text-muted-foreground">No role templates found</div>
        ...
        {roles.map((role) => {
          ...
          return (
            <li key={role.filename}>
              <div className="flex items-start gap-2 px-4 py-3 hover:bg-muted">
                <input
                  type="checkbox"
                  checked={isSelected}
                  onChange={() => toggle(role.name)}
                  className="mt-0.5 w-3.5 h-3.5 rounded accent-[hsl(var(--color-secondary-bright))] cursor-pointer"
                />
                <button
                  type="button"
                ...
                >
                  <div className="text-sm font-medium text-font">{role.name.toUpperCase()}</div>
                  {role.tags.length > 0 && (
                    <div className="text-[10px] text-muted-foreground mt-0.5">{role.tags.slice(0, 3).join(' · ')}</div>
                  )}
                </button>
              </div>
              {isExpanded && role.prompt && (
                <div className="px-4 pb-3 text-[11px] text-muted-foreground leading-relaxed max-h-48 overflow-y-auto whitespace-pre-wrap font-mono">
