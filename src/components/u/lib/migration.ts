// lib/migration.ts — one-shot migration from legacy localStorage to IndexedDB.
// Moved from vault/migration.ts — vault/crypto, storage, types still referenced via vault/ path.
...
import { base64ToBytes, decryptWithKey, encryptUnderMaster } from './vault/crypto'
import { listWallets, putWallet } from './vault/storage'
import { type EncryptedRecord, HKDF_DOMAINS, VaultError, type VaultWallet } from './vault/types'
