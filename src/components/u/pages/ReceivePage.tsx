    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-lg mx-auto px-4 py-8 space-y-6">
        ...
        {error && (
          <div
            role="alert"
            className="rounded-lg border border-destructive/40 bg-destructive/20 px-4 py-3 text-destructive text-sm"
          >
            {error}
          </div>
        )}
        ...
        {/* Loading */}
        {loading && <p className="text-muted-foreground text-sm text-center py-8">Loading wallet…</p>}
        ...
        {!loading && !record && !error && (
          <Card className="bg-card border-border p-8 text-center">
            <CardContent className="pt-0">
              ...
              <p className="text-muted-foreground text-sm mb-4">Create a wallet first to start receiving crypto</p>
              <Button asChild className="bg-primary-bright hover:bg-primary-bright/80 text-black">
        ...
        {!loading && record && (
          <>
            {/* State 1 save banner */}
            {isState1(record) && (
              <div className="rounded-xl border border-gold/40 bg-card/40 px-4 py-3 flex items-start gap-3">
                <span className="text-gold mt-0.5 shrink-0">⚠</span>
                <div className="flex-1 min-w-0">
                  <p className="text-gold text-sm font-medium">Wallet not saved</p>
                  <p className="text-gold/70 text-xs mt-0.5">Save with Touch ID so you don't lose access.</p>
                </div>
                <Button asChild size="sm" className="shrink-0 bg-gold hover:bg-gold/80 text-black">
            ...
            <Card className="bg-card border-border">
              <CardContent className="pt-6 pb-6 flex flex-col items-center gap-4">
                ...
                <div className="w-full text-center">
                  <p className="text-muted-foreground text-xs mb-1">Your Sui address</p>
                ...
                {/* Copy button */}
                <Button
                  className="w-full bg-primary-bright hover:bg-primary-bright/80 text-black"
                  onClick={copyAddress}
                >
                ...
                <details className="w-full">
                  <summary className="text-muted-foreground text-xs cursor-pointer hover:text-foreground text-center">
                    Show full address
                  </summary>
                  <code className="mt-2 block text-muted-foreground text-xs font-mono break-all leading-relaxed text-center">
                    {record.address}
                  </code>
                </details>
              </CardContent>
