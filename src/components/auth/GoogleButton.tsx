...
export function GoogleButton({ redirect = '/app', onError }: Props) {
  ...
  return (
    <button
      ...
      onClick={handle}
      className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-foreground/10 bg-foreground/[0.04] px-4 py-3 text-sm font-medium text-foreground transition hover:bg-foreground/[0.08] disabled:opacity-60"
    >
      {pending ? (
        <>
        ...
        <>
          <svg className="h-4 w-4 shrink-0" viewBox="0 0 24 24" aria-hidden="true">
            <path
              ...
              fill="hsl(var(--color-primary-bright))"
            />
            <path
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
              fill="hsl(var(--color-tertiary-bright))"
            />
            <path
