...
const getStatusBadge = (status: ToolUIPart['state']) => {
  ...
  const icons: Record<ToolUIPart['state'], ReactNode> = {
    ...
    'approval-requested': <ClockIcon className="size-4 text-[hsl(var(--color-gold))]" />,
    'approval-responded': <CheckCircleIcon className="size-4 text-[hsl(var(--color-primary-bright))]" />,
    'output-available': <CheckCircleIcon className="size-4 text-[hsl(var(--color-tertiary-bright))]" />,
    'output-error': <XCircleIcon className="size-4 text-[hsl(var(--color-destructive))]" />,
    'output-denied': <XCircleIcon className="size-4 text-[hsl(var(--color-gold))]" />,
  }

  return (
    <Badge className="gap-1.5 rounded-full text-xs border-border" variant="secondary">
      {icons[status]}
      {labels[status]}
    </Badge>
  )
...
const getToolDescription = (toolName: string, input: any): string => {
  ...
  if (keys.length === 1) {
    ...
    if (typeof value === 'string' || typeof value === 'number') {
      const shortValue = typeof value === 'string' && value.length > 50 ? `${value.slice(0, 50)}...` : value
...
const getOutputSummary = (toolName: string, output: any, errorText?: string): string => {
  const name = toolName?.toLowerCase() || ''
...
export const ToolOutput = ({ className, output, errorText, toolName, ...props }: ToolOutputProps) => {
  if (!(output || errorText)) {
    return null
  }
  ...
  return (
    <div className={cn('space-y-2 p-4', className)} {...props}>
      <div className={cn('text-sm', errorText ? 'text-destructive' : 'text-foreground')}>{summary}</div>
    </div>
  )
}
