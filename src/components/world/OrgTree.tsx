      ...
      <div className="select-none">
        ...
        >
          ...
          {hasChildren ? (
            <span
              className={cn(
                "text-xs w-4 text-slate-500 transition-transform",
          ...
          {/* Group name (bold) */}
          <span className="font-semibold flex-1 truncate text-slate-200">
            {node.name}
          </span>
  ...
  return (
    <div
      className={cn(
        ...
        "hover:bg-slate-800/50 text-sm",
        isFocused &&
          "bg-slate-700 text-white font-semibold shadow-[0_0_12px_rgba(59,130,246,0.4)]"
      )}
      style={{ paddingLeft: `${level * 1 + 0.5}rem` }}
...
export function OrgTree({ className }: OrgTreeProps) {
  ...
  if (loading) {
    return (
      <div
        className={cn(
          "h-full bg-slate-900 border-r border-slate-800 overflow-hidden flex flex-col",
          className
        )}
      >
        <LoadingState />
