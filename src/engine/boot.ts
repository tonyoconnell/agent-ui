...
export const boot = async (complete?: (prompt: string) => Promise<string>, interval = 10_000) => {
  ...
  // Spawn actors from TypeDB
  const actors = await readParsed(`match $u isa actor, has aid $id, has actor-type $kind; select $id, $kind;`).catch(
    () => [],
  )
  for (const u of actors) w.actor(u.id as string, u.kind as string)
  ...
  // Register chain bridge actors (evm, sol, btc — no bridge:sui, Sui is home)
  registerBridges(w)

  // Register pay actor (Sui-native USDC commerce, L4 Marketplace)
  registerPayUnit(w)
  ...
  // This actor marks each tag path proportionally to the rubric score.
  ...
  // Register builder actor with wave chain (W1→W4)
