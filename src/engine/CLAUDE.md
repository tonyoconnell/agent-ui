...
**Context:** [DSL.md](one/DSL.md) defines the signal grammar. [dictionary.md](dictionary.md) defines the names. [routing.md](routing.md) — this folder IS routing.md in code: `loop.ts` implements the tick loop (select→ask→mark/warn→fade), `persist.ts` implements the deterministic sandwich (toxic→capable→execute→outcome), `world.ts` implements the formula (`weight = 1 + max(0, strength - resistance) × sensitivity`). Four outcomes: `{result}` → mark(chainDepth), `{timeout}` → neutral, `{dissolved}` → warn(0.5), `(none)` → warn(1). [rubrics.md](rubrics.md) defines quality scoring as tagged edges. [patterns.md](one/patterns.md) defines the 10 emergent patterns. [lifecycle.md](one/lifecycle.md) defines into/through/out. [speed.md](one/speed.md) defines the benchmarks that make the learning rate possible. [buy-and-sell.md](buy-and-sell.md) — `data.weight` is the payment amount; `mark(edge, weight)` settles the trade and deposits pheromone in one call (L4). [revenue.md](one/revenue.md) — the five revenue layers the engine drives (routing → discovery → infra → marketplace → intelligence).
