...
export const openrouter = (apiKey: string, model = 'google/gemma-4-26b-a4b-it') =>
  (prompt: string, ctx?: Record<string, unknown>) =>
    ...
    }).then(r => r.json()).then(d => (d as any).choices[0].message.content)
...
export const anthropic = (apiKey: string) => (prompt: string, ctx?: Record<string, unknown>) =>
  ...
  }).then(r => r.json()).then(d => (d as any).content[0].text)
