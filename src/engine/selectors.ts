 ...
 */
export const first = <T>(): Selector<T> => (items) => items[0] || null

/**
 * Pick a random item.
 */
export const random = <T>(): Selector<T> => (items) =>
 ...
 */
export const byPheromone = <T extends WithPath>(net: World): Selector<T> => (items) => {
 ...
 */
export const weighted = <T extends WithPath>(
  net: World,
  explore = 0.3
): Selector<T> => (items) => {
  if (!items.length) return null
 ...
 */
export const byPriority = <T extends WithPriority>(): Selector<T> => (items) => {
  if (!items.length) return null
 ...
 */
export const priorityWeighted = <T extends WithPriority>(explore = 0.2): Selector<T> => (items) => {
  if (!items.length) return null
  if (Math.random() < explore) return items[Math.floor(Math.random() * items.length)]

  const weights = items.map(item => {
    const p = priorityValue(item.priority)
    return Math.pow(2, 3 - p)  // P0=8, P1=4, P2=2, P3=1
  })
 ...
 */
export const bySuccessRate = <T extends WithSuccessRate>(): Selector<T> => (items) => {
  if (!items.length) return null
 ...
 */
export const byNeed = <T extends WithSuccessRate>(): Selector<T> => (items) => {
  if (!items.length) return null
...
export const fallback = <T>(
  primary: Selector<T>,
  secondary: Selector<T>
): Selector<T> => (items) => primary(items) || secondary(items)
...
export const filtered = <T>(
  ...
  selector: Selector<T>
): Selector<T> => (items) => selector(items.filter(filter))
