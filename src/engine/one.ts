...
const mark = (p: string, n = 1) => { s[p] = (s[p] || 0) + n }
const warn = (p: string, n = 1) => { r[p] = (r[p] || 0) + n }
const net = (p: string) => (s[p] || 0) - (r[p] || 0)

const on = (id: string, fn: H) => { h[id] = fn }
...
const tick = async (from = 'entry'): Promise<O & { p?: string; ms: number }> => {
  ...
  if (!fn) { warn(p, 0.5); return { d: true, p, ms: Date.now() - t0 } }
...
const timer = (signal: string, ms: number) => {
  const tick = () => { emit(signal); setTimeout(tick, ms) }
  setTimeout(tick, ms)
}
...
const sop = (target: string, prereqs: string[]) => {
  ...
  on(target, async (d, e) => {
    for (const p of prereqs) {
      ...
      if (result.d) { warn(`sop:${target}`, 1); return { blocked: p } }
    }
    mark(`sop:${target}`, 1)
    return original?.(d, e)
  })
...
const getCtx = (...keys: string[]) => keys.map(k => ctx[k] || '').filter(Boolean).join('\n---\n')
...
const learnRate = () => 1 + Math.log10(cycle + 1)  // grows with experience

const fade = () => {
  for (const p in s) { s[p] *= (1 - decay); if (s[p] < 0.01) delete s[p] }
  for (const p in r) { r[p] *= (1 - decay * 2); if (r[p] < 0.01) delete r[p] }  // forgive faster
}
...
// Top paths
const highways = (n = 10) => Object.keys(s)
  .map(p => ({ p, n: net(p) }))
  .filter(x => x.n > 0)
  .sort((a, b) => b.n - a.n)
  .slice(0, n)
...
export const one = {
  ...
  loadCtx, getCtx,
  // Learning
  mark, warn, fade, highway, toxic, highways,
  // State (read-only)
  get q() { return q.length },
  get cycle() { return cycle },
  get handlers() { return Object.keys(h) },
}
