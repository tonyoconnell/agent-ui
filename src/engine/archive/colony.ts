...
const world = (): World => {
  ...
  const fade = (rate: number = 0.1): void => {
    ...
    for (const edge in alarm) {
      alarm[edge] *= 1 - rate * 2
      if (alarm[edge] < 0.01) delete alarm[edge]
    }
  }
  ...
  const signal = ({ receiver, receive, data, payload, callback }: Signal, from: string = 'entry'): Promise<unknown> => {
    ...
    return target({ receive, data: data ?? payload, callback })
      .then((result) => {
        return result
      })
      .catch((error) => {
...
export type { ColonyError, Edge, UnitJSON, World }
export { world }
