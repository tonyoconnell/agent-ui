...
const unit = (signal: Signal, route?: RouteFn): Unit => {
  ...
  const receive = (({ receive: t, data, payload, callback }: Signal): Promise<unknown> => {
    ...
    return s[t!](data ?? payload)
      ...
      .catch((error) => Promise.reject({ id, target: t, data: data ?? payload, error } as UnitError))
  }) as Unit

  receive.assign = <P, R>(n: string, f: (data: P) => R): Unit => ((s[n] = (d) => Promise.resolve(f(d as P))), receive)
...
export type { RouteFn, Signal, Unit, UnitError }
export { unit }
