...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    writeSilent(
      `match $g isa group, has group-id "${groupId}"; $b isa unit, has uid "${buyerUid}"; insert (member: $b, group: $g) isa membership, has member-role "buyer";`,
    )
    writeSilent(
      `match $g isa group, has group-id "${groupId}"; $p isa unit, has uid "${providerUid}"; insert (member: $p, group: $g) isa membership, has member-role "provider";`,
    )
  } catch {
