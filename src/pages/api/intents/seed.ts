...
import { seedIntents, type Intent } from '@/engine/intent'

export const POST: APIRoute = async ({ request, locals }) => {
  const { intents } = await request.json() as { intents: Intent[] }
  if (!Array.isArray(intents)) return new Response(JSON.stringify({ ok: false, error: 'intents must be array' }), { status: 400 })
  ...
  return new Response(JSON.stringify({ ok: true, seeded: intents.length }), { headers: { 'Content-Type': 'application/json' } })
}
