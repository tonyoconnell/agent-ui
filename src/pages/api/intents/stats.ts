...
export const GET: APIRoute = async ({ locals }) => {
  ...
  const [intents, queries] = await Promise.all([
    env.DB.prepare('SELECT name, label, hit_count FROM intents ORDER BY hit_count DESC').all<{ name: string; label: string; hit_count: number }>(),
    env.DB.prepare(`
      SELECT intent, resolver, COUNT(*) as count, SUM(cached) as hits
      FROM intent_queries WHERE ts > ?
      GROUP BY intent, resolver
    `).bind(Date.now() - 7 * 86_400_000).all<{ intent: string | null; resolver: string; count: number; hits: number }>(),
  ])
  ...
  return new Response(JSON.stringify({ ok: true, stats, unknownCount }), { headers: { 'Content-Type': 'application/json' } })
}
