...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  if (!uid || !UID_RE.test(uid) || uid.length > 255) {
    return badRequest('uid must match /^[a-zA-Z0-9_:.-]+$/ and be ≤255 chars')
  }

  const address = typeof body.address === 'string' ? body.address : null
  if (!address || !ADDRESS_RE.test(address)) {
    return badRequest('address must match /^0x[0-9a-fA-F]{64}$/')
  }

  const ciphertextB64 = typeof body.ciphertextB64 === 'string' ? body.ciphertextB64 : null
  if (!ciphertextB64) {
    return badRequest('ciphertextB64 is required')
  }
  ...
  if (!ciphertextBytes || ciphertextBytes.length < 48) {
    ...
    return badRequest('ciphertextB64 must decode to ≥48 bytes (seed + GCM auth tag)')
  }

  const ivB64 = typeof body.ivB64 === 'string' ? body.ivB64 : null
  if (!ivB64) {
    return badRequest('ivB64 is required')
  }
  const ivBytes = decodeBase64url(ivB64)
  if (!ivBytes || ivBytes.length !== 12) {
    return badRequest('ivB64 must decode to exactly 12 bytes (AES-GCM nonce)')
  }
  ...
  if (!db) {
    return serviceUnavailable('d1-unavailable', 'D1 binding unavailable')
  }
  ...
  try {
    ...
    if (existing && existing.n > 0) {
      return err(409, 'already-registered', undefined, { uid })
    }
  } catch (e) {
    console.error('[agents/register] D1 count check failed', e)
    return err(500, 'd1-failed', 'count check failed')
  }
  ...
  try {
    ...
    console.error('[agents/register] bearer mint failed', e)
    return err(500, 'typedb-failed', 'key generation failed')
  }
  ...
  try {
    ...
    return err(500, 'typedb-failed', 'api-key insert failed')
  }
  ...
  try {
    ...
    console.error('[agents/register] D1 INSERT failed', e)
    return err(500, 'd1-failed', 'agent_wallet insert failed')
  }

  // ── 9. Return receipt — bearer shown ONCE ────────────────────────────────────
  return ok({ uid, address, bearer, keyId, kdfVersion }, { headers: { 'X-ONE-Protocol': 'v1' } })
}
