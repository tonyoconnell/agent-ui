...
function calculateSummary(data: any[], fields?: string[]): any {
  const numericFields = fields || Object.keys(data[0] || {}).filter((key) => typeof data[0][key] === 'number')
...
function calculateCorrelation(data: any[], fields?: string[]): any {
  ...
  for (let i = 0; i < numericFields.length; i++) {
    for (let j = i + 1; j < numericFields.length; j++) {
      ...
      const cov = values1.reduce((acc, v1, idx) => acc + (v1 - mean1) * (values2[idx] - mean2), 0) / n

      const std1 = Math.sqrt(values1.reduce((acc, v) => acc + (v - mean1) ** 2, 0) / n)
      const std2 = Math.sqrt(values2.reduce((acc, v) => acc + (v - mean2) ** 2, 0) / n)
...
export const dataAnalyzerTool: Tool = {
  name: 'analyze_data',
  description:
    'Analyze datasets to compute statistics, detect patterns, find outliers, and identify trends. Supports summary stats (mean, median, mode), distribution analysis, correlation analysis, outlier detection, and trend identification.',
  category: 'data',
