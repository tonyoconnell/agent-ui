...
export const latestQuotesTool: Tool = {
  name: 'cmc_get_quotes',
  description:
    'Get latest cryptocurrency quotes from CoinMarketCap. Returns price, market cap, volume, and price changes (1h, 24h, 7d, 30d). Use token symbols (e.g., "BTC", "ETH", "VIRTUAL").',
...
export const tokenMetadataTool: Tool = {
  ...
  description:
    'Get cryptocurrency metadata from CoinMarketCap including description, links, platform info, tags, and audit status.',
  category: 'integration',
...
const topCryptosParams = z.object({
  ...
  convert: z.string().default('USD').describe('Quote currency'),
  sortBy: z
    .enum(['market_cap', 'volume_24h', 'percent_change_24h', 'percent_change_7d'])
    .default('market_cap')
    .describe('Sort field'),
...
export const topCryptosTool: Tool = {
  ...
  description:
    'Get top cryptocurrencies from CoinMarketCap ranked by market cap. Useful for market overview and comparisons.',
  category: 'integration',
...
export const globalMetricsTool: Tool = {
  ...
  description:
    'Get global cryptocurrency market metrics from CoinMarketCap including total market cap, BTC dominance, DeFi stats, and stablecoin metrics.',
  category: 'integration',
...
async function getCategoryTokens(params: z.infer<typeof categoryTokensParams>) {
  ...
  try {
    ...
    const categoryInfo = categoriesData.data?.find(
      (cat: any) =>
        cat.id === category || cat.slug === category || cat.name.toLowerCase().includes(category.toLowerCase()),
    )
...
export const categoryTokensTool: Tool = {
  ...
  description:
    'Get tokens by category from CoinMarketCap. Useful for analyzing specific sectors like "ai-big-data", "defi", "layer-1", "gaming".',
  category: 'integration',
