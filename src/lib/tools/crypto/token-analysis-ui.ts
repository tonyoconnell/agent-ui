...
import { tokenKnowledgeTool, getAvailableTokens, hasTokenKnowledge } from './knowledge';
import type { TokenAnalysisData, ComponentType } from '@/components/crypto/TokenAnalysisUI';
...
const renderTokenAnalysisParams = z.object({
  ...
  components: z.array(componentTypeSchema).optional().describe('Which UI components to render. Defaults to price_chart, metrics, risk_gauge, flags'),
  compact: z.boolean().optional().default(true).describe('Use compact mode for chat (true) or full mode for pages (false)'),
});
...
function extractRiskScore(content: string): { score: number; grade: string; recommendation: string } {
  ...
  return {
    score: scoreMatch ? parseInt(scoreMatch[1]) : 45,
...
function extractMarketData(content: string): {
  ...
  if (holdersMatch) defaults.holders = parseInt(holdersMatch[1].replace(/,/g, ''));
...
function extractFlags(content: string): { redFlags: string[]; greenFlags: string[] } {
  ...
  const greenSection = content.match(/(?:green\s*flags?|positives?|strengths?|bulls?)[:\s]*\n((?:[-•*]\s*[^\n]+\n?)+)/gi);
  if (greenSection) {
...
function extractThesis(content: string): TokenAnalysisData['thesis'] | undefined {
  ...
  return {
    ...
    bull: {
      ...
      probability: bullProbMatch ? parseInt(bullProbMatch[1]) : 30,
    },
    bear: {
      target: bearMatch ? `$${bearMatch[1]}` : '$0.00',
      probability: bearProbMatch ? parseInt(bearProbMatch[1]) : 20,
    },
  };
}
...
async function renderTokenAnalysis(
  ...
  const tokenName = frontmatter.title?.replace(/Token Analysis[:\s]*/i, '').trim()
    || symbol.toUpperCase();
