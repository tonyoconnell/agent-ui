...
async function formatDateTool(params: z.infer<typeof dateFormatterParams>) {
  ...
  try {
    ...
    throw new Error(`Date operation failed: ${error instanceof Error ? error.message : 'Unknown error'}`)
  }
}
...
export const dateFormatterTool: Tool = {
  name: 'format_date',
  description:
    'Format and manipulate dates. Operations: format (custom format), relative (time ago), add (add time), subtract (subtract time), diff (difference between dates), parse (validate date), timestamp (Unix timestamp), iso (ISO 8601). Format tokens: YYYY, MM, DD, HH, mm, ss, MMM, MMMM.',
  category: 'utility',
