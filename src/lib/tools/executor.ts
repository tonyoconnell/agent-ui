...
function logExecution(log: ExecutionLog): void {
  ...
  if (typeof window !== 'undefined') {
    window.dispatchEvent(new CustomEvent('tool-execution', {
      detail: log
    }));
  }
}
...
export async function executeToolsBatch(
  ...
  return Promise.all(
    executions.map(({ toolName, params, context }) =>
...
export function getToolStatistics(toolName?: string): ToolStatistics {
  const logs = toolName
    ? executionLogs.filter(log => log.toolName === toolName)
    : executionLogs;

  const totalExecutions = logs.length;
  const successCount = logs.filter(log =>
    log.state === 'completed' || log.state === 'cached'
  ).length;
  const failureCount = logs.filter(log =>
    log.state === 'failed' || log.state === 'timeout'
  ).length;
