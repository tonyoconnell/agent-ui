...
import { type AllowedTargetSet, isAllowed, parseAllowedTargets } from '../allowed-targets'
...
describe('isAllowed', () => {
  ...
  it('returns true when all targets are allowed (mixed kinds)', () => {
    expect(isAllowed(['0xdeadbeef', '0xa5e6bdda::module::fn', '0x2::coin::transfer'], baseSet)).toBe(true)
  })
...
describe('parseAllowedTargets — invalid input', () => {
  ...
  it('throws when createdAt is missing', () => expectScopeViolation({ version: 1, targets: [], updatedAt: 'x' }))
