...
describe("getConsensusSuiPrice", () => {
  ...
  it("throws when Pyth feed is stale", async () => {
    const staleTs = STALE()
    const freshTs = FRESH()
    mockBothFeeds(3.40, staleTs, 3.40, freshTs)
  ...
  it("throws when CoinGecko feed is stale", async () => {
    const freshTs = FRESH()
    const staleTs = STALE()
    mockBothFeeds(3.40, freshTs, 3.40, staleTs)
  ...
  it("deviation error message includes the percentage", async () => {
    const ts = FRESH()
    mockBothFeeds(3.00, ts, 3.10, ts)  // ~3.33% deviation
  ...
  it("returns single feed price when only Pyth is available", async () => {
    ...
    vi.mocked(fetch)
      ...
      .mockResolvedValueOnce({
        ok: true,
        json: async () => pythResponse(3.40, ts),
      } as Response)
      // CoinGecko fails
      .mockRejectedValueOnce(new Error("network error"))

    const price = await getConsensusSuiPrice()
    expect(price).toBeCloseTo(3.40, 4)
  })
  ...
  it("throws when no feeds are available", async () => {
    ...
    await expect(getConsensusSuiPrice()).rejects.toThrow(
      "oracle: no price feeds available"
    )
  })
  ...
  it("throws when the only available feed is stale", async () => {
    ...
    vi.mocked(fetch)
      ...
      .mockResolvedValueOnce({
        ok: true,
        json: async () => geckoResponse(3.40, staleTs),
      } as Response)
...
describe("mistToUsd", () => {
  it("converts 1 SUI (10^9 MIST) at price $3.40", () => {
    expect(mistToUsd(1_000_000_000n, 3.40)).toBeCloseTo(3.40, 8)
  })

  it("converts 0.5 SUI at price $4.00", () => {
    expect(mistToUsd(500_000_000n, 4.00)).toBeCloseTo(2.00, 8)
  })

  it("converts 0 MIST to $0", () => {
    expect(mistToUsd(0n, 3.40)).toBe(0)
  })

  it("handles large MIST amounts without overflow", () => {
    // 1,000 SUI at $3.00
    const result = mistToUsd(1_000_000_000_000n, 3.00)
