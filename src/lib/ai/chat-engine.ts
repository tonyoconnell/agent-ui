...
import { type Message, toAPIMessages } from './message-formatter'
 ...
 */
export function estimateMessageCost(messages: Message[], model: string): { tokens: number; cost: number } {
...
export function validateChatConfig(config: ChatConfig): {
  ...
  const premiumModels = ['openai/gpt-4o', 'openai/gpt-5', 'anthropic/claude-sonnet-4.5', 'anthropic/claude-3-opus']
