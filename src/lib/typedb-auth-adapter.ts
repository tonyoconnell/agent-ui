...
function escapeTQL(value: string): string {
  return value.replace(/\\/g, '\\\\').replace(/"/g, '\\"')
}

function toTQL(value: unknown): string {
  if (value === null || value === undefined) return ''
  if (typeof value === 'string') return `"${escapeTQL(value)}"`
  if (typeof value === 'number') return value.toString()
  if (typeof value === 'boolean') return value.toString()
  if (value instanceof Date) return value.getTime().toString()
  return `"${escapeTQL(String(value))}"`
}

function fieldToAttr(field: string): string {
  return FIELD_TO_ATTR[field] || `auth-${field.replace(/([A-Z])/g, '-$1').toLowerCase()}`
}
  ...
  return createAdapter({
    ...
    adapter: () => ({
      ...
      findMany: async ({ model, where, limit: lim, offset }) => {
        ...
        let conditions = ''
        if (where?.length) {
          conditions = where.map((w) => `$e has ${fieldToAttr(w.field)} ${toTQL(w.value)};`).join(' ')
        }
        ...
        const ids = idAnswers.map((a) => a.data?.id?.value).filter(Boolean)

        const results: any[] = []
        for (const id of ids) {
          const entityTql = `match $e isa ${entity}, has auth-id "${escapeTQL(id)}"; $e has $attr; select $attr;`
      ...
      count: async ({ model, where }) => {
        ...
        try {
          const answers = await query(config, `match $e isa ${entity}; ${conditions} reduce $count = count;`, false)
      ...
      update: async ({ model, where, update: updates }) => {
        ...
        const conditions = where.map((w) => `$e has ${fieldToAttr(w.field)} ${toTQL(w.value)};`).join(' ')
        ...
        for (const [field, value] of Object.entries(updates as Record<string, unknown>)) {
          ...
          try {
            await query(config, `match $e isa ${entity}; ${conditions} $e has ${attr} $old; delete $old of $e;`, true)
          } catch {
            /* attr may not exist */
          }

          if (value !== null) {
            await query(config, `match $e isa ${entity}; ${conditions} insert $e has ${attr} ${toTQL(value)};`, true)
          }
        }
      ...
      delete: async ({ model, where }) => {
        ...
        await query(config, `match $e isa ${entity}; ${conditions} delete $e;`, true)
      },
