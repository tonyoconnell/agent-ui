...
async function getSuiTransactions(address: string): Promise<Transaction[]> {
  ...
  const fromData = (await fromResponse.json()) as Record<string, any>
  ...
  const toData = (await toResponse.json()) as Record<string, any>
  ...
  for (const tx of combined) {
    ...
    const sender = ((tx.transaction?.data as Record<string, any>)?.sender as string) || ''
...
async function getEthTransactions(address: string): Promise<Transaction[]> {
  ...
  return (data.result as Record<string, string>[]).map((tx) => ({
    id: tx.hash,
    hash: tx.hash,
...
async function getSolTransactions(address: string): Promise<Transaction[]> {
  ...
  return (data.result as Record<string, any>[]).map((sig) => ({
...
async function getBtcTransactions(address: string): Promise<Transaction[]> {
  ...
  return data.slice(0, 25).map((tx: Record<string, unknown>) => {
    const status = tx.status as Record<string, any> | undefined
    return {
