...
revenue** ([revenue.md](revenue.md)). It also wrote a signal, so the
...
That's the entire engine in miniature. See [patterns.md](patterns.md).
...
documented in [patterns.md § The Four Outcome Types](patterns.md). Never
...
- [lifecycle.md](lifecycle.md) — the substrate's career view (register → harden)
...
- [revenue.md](revenue.md) — five fee layers, what each command pays
- [patterns.md](patterns.md) — closed loop, deterministic sandwich, four outcomes
- [sdk.md](sdk.md) — programmatic surface (same verbs, TypeScript calls)
