...
| **alarm** | This route fails | `warn()` after failure |

Strength minus alarm = how much the system trusts this route.

When strength gets high enough, the path becomes a **highway**.
When alarm overtakes strength, the path goes **toxic**.
When nothing flows, the path **fades** and dissolves.

```
    mark    ──→  strength++  ──→  highway emerges
    warn    ──→  alarm++     ──→  path goes toxic
    fade    ──→  both decay  ──→  stale paths dissolve
    nothing ──→  silence     ──→  signal dissolves (that's ok)
```

Alarm fades twice as fast as strength. The system forgives
...
Units don't exist alone. They live in a **colony**.

A colony is the world signals move through. It holds all the units,
...
```
    ...
    │    └───────┘  str: 47  └──────────┘  str: 31      │
    │                                                     │
    │    scent: { "scout→analyst": 47, ... }             │
    │    alarm: { "scout→bad-route": 8, ... }            │
    │                                                     │
    └─────────────────────────────────────────────────────┘
```

### What a Colony Can Do

| Method | What it does |
|--------|-------------|
| `spawn(id)` | Create a new unit |
| `despawn(id)` | Remove a unit (trails remain, fade naturally) |
| `signal(sig)` | Send a signal into the world |
| `mark(path)` | Strengthen a path |
| `warn(path)` | Weaken a path |
| `sense(path)` | Read the strength |
| `danger(path)` | Read the alarm |
...
The colony is the **substrate** — the raw medium. It doesn't know about
groups or economics or knowledge. It just knows: units, signals, pheromone.

---

## The World

On top of the colony, there's the **world**. The world adds six dimensions
that turn raw signal flow into something you can build a platform on.

```
    ┌─────────────────────────────────────────────┐
    │                  WORLD                       │
    │          (6 dimensions on colony)            │
    │                                              │
    ...
    │    ┌──────────────────────────────────────┐  │
    │    │            COLONY                    │  │
    │    │   units + signals + pheromone        │  │
    │    └──────────────────────────────────────┘  │
    │                                              │
    └─────────────────────────────────────────────┘
```

The colony is the substrate. The world is what grows on it.
...
A **swarm** is a container. A team. An org. A persona. A DAO.
Units live inside swarms. Swarms can nest inside other swarms.
...
| Field | What it is |
|-------|-----------|
| `sid` | Swarm identifier |
| `swarm-type` | `"persona"`, `"team"`, `"colony"`, `"dao"` |
| `purpose` | Why this group exists |

**Relation:** `membership` — connects units to swarms.
**Relation:** `hierarchy` — nests swarms inside swarms.
...
```
    ...
    repelled ──── alarm pheromone dominates
                  (ants avoid this)
```
...
Both carry the same dual weight: strength and alarm.
...
```
    ...
    │         │                                               │
    │         ▼                                               │
    │   world()                      the 6 dimensions          │
    │     .group()                   scope & isolation          │
    │     .actor()                   who acts                   │
    │     .thing()                   what exists                │
    │     .flow()                    weighted connections       │
    │     .crystallize()             permanent knowledge        │
...
Built on the same substrate. Same signals. Same pheromone.
...
because the substrate learns the routes and the LLM is no longer needed.
...
### Agentverse — 2 Million Agents as Colony

The Fetch.ai agent registry, modeled as a colony:

```
    register(meta)  ── add an agent to the colony
...
On startup, `load()` hydrates the colony from the database.
The in-memory colony and TypeDB stay in sync.
...
The colony routes toward value. Not because it was told to.
...
The trail evaporates. No exception thrown. The colony moves on.
...
```
    ...
    │   the colony learns which routes work.          │
    │                                                 │
    ├─────────────────────────────────────────────────┤
    │                                                 │
    │   AGENT LEARNING                                │
    │   The individual gets smarter.                  │
    │   The colony provides the data.                 │
    │                                                 │
    ...
    │       → generation++                            │
    │                                                 │
    │   The substrate says "you're struggling."       │
...
| `is_repelled(task)` | Alarm > trail pheromone |
...
| `collaborators(unit)` | Peers in the same swarm |
...
Same system. Different words. The substrate doesn't care.
...
```
    ...
    │ colony   │ nest   │ network │ team   │ office │watershed│ network │
    ...
    │ spawn    │ hatch  │ grow    │ hire   │ open   │ dig     │ tune    │
    └──────────┴────────┴─────────┴────────┴────────┴─────────┴─────────┘
```
...
|------|------|------:|------|
| Signal, Unit, Colony | `src/engine/substrate.ts` | 180 | The substrate |
| World | `src/engine/one.ts` | 70 | 6 dimensions |
| Persist | `src/engine/persist.ts` | 90 | TypeDB sync |
| ASI | `src/engine/asi.ts` | 84 | Orchestrator |
| LLM | `src/engine/llm.ts` | 40 | AI as unit |
| Agentverse | `src/engine/agentverse.ts` | 83 | 2M agents |
| Tick | `src/engine/loop.ts` | 103 | Growth cycle |
| Schema | `src/schema/one.tql` | 550 | TypeDB truth |
| Skins | `src/schema/skins.tql` | 300 | Metaphor layer |
| Sui | `src/schema/sui.tql` | — | On-chain mirror |
...
```
    ...
    UNIT ──────────── receives signals, runs tasks
       │
       ▼
    COLONY ────────── units + paths + pheromone
       │
       ▼
    WORLD ─────────── 6 dimensions on colony
       │
       ▼
    TYPEDB ────────── persist + route + classify
       │
       ▼
    ASI ───────────── orchestrate + learn + evolve
       │
       ▼
    TICK ──────────── sense → select → execute → update → decay → evolve → crystallize
       │
       ▼
...
Frontiers become objectives. The colony pursues them.
