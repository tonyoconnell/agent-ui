...
This document teaches you everything about ONE — from the signal primitive to multi-species colonies to on-chain crystallization.
...
colony()
  ...
  .mark(path, strength?)  // drop pheromone on path
...
|--------|--------|--------|--------|-------|--------|-----------|
| **Substrate** | colonies | units | signals | pheromone trails | traversals | highways |
...
- TypeDB inference classifies paths automatically: `highway` (weight >= 50), `fading` (weight < 5), `toxic` (alarm > weight)
...
```
1. SIGNAL
   colony.signal({ receiver: 'scout:observe', data: { tick: 42500 } })
...
6. CONTINUE (if .then defined)
   ...
   colony.signal(next_signal, from="scout:observe")
```
...
colony.signal(sig)              // mark() called automatically: +1

// Periodic decay
colony.fade(0.1)                // all paths × 0.9

// What emerged
colony.highways(10)             // top 10 paths by weight
```
...
// Resolve outcome
colony.resolve('agent', 'task', true)  // 10 → 12 (+20%)
colony.resolve('agent', 'task', false) // 10 → 9 (-10%)
...
colony.resolve(agent, task, true)           // ×1.2
colony.resolve(agent, task, false)          // ×0.9
...
|---------|-----------|-------------|----------|
| Hermes | `"agent"` | Deep (local colony + sync) | Long-running, stateful |
...
**Deep** — Agent imports substrate, runs colony locally:
...
const config = await generateObject({
  ...
  prompt: `Given: ${JSON.stringify(highways)}. Configure optimal colony.`
})
...
const result = streamText({
  tools: {
    signal: tool({ description: 'Send signal', parameters: signalSchema,
      execute: ({ receiver, data }) => colony.signal({ receiver, data }) }),
    drop: tool({ description: 'Strengthen path', parameters: dropSchema,
      execute: ({ path, strength }) => colony.mark(path, strength) }),
    discover: tool({ description: 'Find best agent', parameters: discoverSchema,
      execute: ({ task }) => colony.follow(task) }),
  }
})
```
...
```
Opus     → Architects (analyze scent graph, propose splits)
...
// Load scent graph from TypeDB on restart
await one.load()
```
...
net.spawn('payment')
  .on('process', async (data, emit, ctx) => {
    ...
    ok
      ...
      : (net.alarm?.(`${ctx.from}→${ctx.self}`),       // weaken
...
| Group   | colony    | network    | team      | office     | watershed  | network   |
| Signal  | pheromone | spike      | task      | letter     | drop       | pulse     |
| Path    | trail     | synapse    | workflow  | route      | channel    | frequency |
| Drop    | deposit   | potentiate | commend   | stamp      | carve      | boost     |
...
```
src/engine/
├── substrate.ts   (70)  — Unit + Colony + scent graph
...
- [signal.md](signal.md) — The universal primitive deep dive
...
- [swarm.md](swarm.md) — Swarm coordination patterns
- [hermes-agent.md](hermes-agent.md) — Multi-species agent integration
- [emergence.md](emergence.md) — How complexity emerges from simple rules
