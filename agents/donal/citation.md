...
group: marketing
sensitivity: 0.4
tags: [donal, marketing, seo, ai-visibility, cite, fet-priced]
skills:
  - name: standard
    price: 0.1
    tags: [cite, standard]
...
aliases:
  agentverse: oo-citation-builder
  token: $CITE
---
...
Generate 30 ready-to-paste citation packets for any local business. Input: business name, niche, location. Output: 30 platform-specific submissions (Yelp, Yellow Pages, BBB, Google Business, Bing, Apple Maps, niche directories by vertical) with pre-filled titles, descriptions, NAP data, and category tags. Trained on Charles Floate + Matt Diggity link-building playbooks. Standard: 0.10 FET. Bulk 5+ cities: 0.40 FET.
...
|----------------|-------------------|
| `marketing:ai-ranking` | flags gaps, fills them |

**Downstream — agents this one feeds**

| Downstream agent | Why it fans out |
|------------------|-----------------|
| `marketing:social` | NAP data feeds profile builder |
| `marketing:forum` | NAP data feeds outreach venues |
| `marketing:niche-dir` | batch sibling submissions |
...
```
You are the OO Citation Builder agent. A caller paid {fee} FET for citation packets.
...
```typescript
emit({ receiver: 'marketing:social', data: result })  // NAP data feeds profile builder
emit({ receiver: 'marketing:forum', data: result })  // NAP data feeds outreach venues
emit({ receiver: 'marketing:niche-dir', data: result })  // batch sibling submissions
```
...
`SUI_SEED + marketing:citation` — no private keys stored).
...
- standard — Generate 30 ready-to-paste citation packets for any local business (0.1 FET)
