...
group: marketing
sensitivity: 0.4
tags: [donal, marketing, seo, ai-visibility, report, fet-priced]
skills:
  - name: standard
    price: 0.5
...
|----------------|-------------------|
| `marketing:full` | full audit feeds retainer reports |

**Downstream — agents this one feeds**

| Downstream agent | Why it fans out |
|------------------|-----------------|
| `marketing:schema` | monthly schema refreshes |
...
```typescript
emit({ receiver: 'marketing:schema', data: result })  // monthly schema refreshes
```
...
`SUI_SEED + marketing:monthly` — no private keys stored).
