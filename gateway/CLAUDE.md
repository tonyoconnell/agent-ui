...
**Context:** [speed.md](one/speed.md) — gateway `<10ms` p50, why it matters for learning rate. [routing.md](routing.md) — the sandwich that determines when TypeDB is queried vs skipped; gateway enforces the PRE checks before every proxied query. [revenue.md](one/revenue.md) — every signal relayed here is a Layer 1 routing fee event; gateway latency directly determines the max transaction rate. [DSL.md](one/DSL.md) — signal grammar that the gateway relays without parsing; receiver/data shape is opaque to the proxy layer. [dictionary.md](dictionary.md) — canonical names used in the `/broadcast` message type allowlist (`complete`, `unblock`, `mark`, `warn`, `task-update`, `sync`). [lifecycle.md](one/lifecycle.md) — gateway is the infra layer; register/signal/highway/harden all pass through here as HTTP or WebSocket traffic. [buy-and-sell.md](buy-and-sell.md) — EXECUTE and SETTLE steps transit the gateway via `/typedb/query` relay; `/api/pay` is proxied here. [patterns.md](one/patterns.md) — gateway implements the deterministic sandwich pre-check for every proxied TypeDB query (toxic → dissolve before reaching the brain). [rubrics.md](rubrics.md) — integration tests score against fit/form/truth/taste; 11/11 deterministic passes are the rubric baseline for this layer.
